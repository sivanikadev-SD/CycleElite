...
if (indexContent.includes('What sensors can I connect?')) {
  ...
  indexContent = indexContent.replace(/(<span class="faq-title">What sensors can I connect\?<\/span>[\s\S]*?<\/div>\s*<\/div>\s*<\/div>)/, "$1" + newFaqIndex);
...
if (progContent.includes('Can I switch plans mid-way through a block?')) {
  progContent = progContent.replace(/(<span class="faq-title">Can I switch plans mid-way through a block\?<\/span>[\s\S]*?<\/div>\s*<\/div>\s*<\/div>)/, "$1" + newFaqProg);
...
if (priceContent.includes('How do I get matched with a coach?')) {
  priceContent = priceContent.replace(/(<span class="faq-title">How do I get matched with a coach\?<\/span>[\s\S]*?<\/div>\s*<\/div>\s*<\/div>)/, "$1" + newFaqPrice);
